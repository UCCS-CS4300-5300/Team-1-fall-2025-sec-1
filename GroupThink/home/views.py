"""Views for the GroupThink home application."""
import json
import os
import threading
import uuid
...
def create_workspace(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                f'Workspace "{workspace.name}" created! Invite code: {workspace.invite_code}'
            )
...
def update_meeting_status(request, meeting_id):
    ...
    if request.method == 'POST':
        ...
        if new_status in ['not_started', 'live', 'ended']:
            ...
            meeting.save()
            messages.success(
                request, f'Meeting status updated to "{meeting.get_status_display()}".'
            )
        else:
...
def update_task_status(request, task_id):
    ...
    # Check if user has permission (assigned to them, admin of workspace, or creator)
...
# For receiving data from JaaS regarding transcripts
@csrf_exempt
def jaas_webhook(request):
    """Handle JaaS webhook events for transcriptions and recordings."""
    try:
...
def get_transcript(request, room_name: str):
    """Get transcript data for a meeting room."""
...
def my_recordings(request):
    """Display recordings for meetings the user participated in."""
    user = request.user
...
def recording_detail(request, pk):
    """Display details for a specific recording."""
...
def get_chat_messages(request, workspace_id):
    ...
    messages_qs = workspace.chat_messages.filter(id__gt=since_id).select_related(
