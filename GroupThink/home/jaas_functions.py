"""JaaS (Jitsi as a Service) integration functions for JWT generation."""
import os
import time
...
def _load_private_key(key_file_name="jaas_private.pem"):
    """Load the JaaS private key from environment or file."""
    ...
    if pem_env:
        ...
        if not pem.startswith("-----BEGIN") or "PRIVATE KEY-----" not in pem:
            raise TypeError(
                "JAAS_PRIVATE_KEY env does not contain a valid PEM private key"
            )
        return pem
    ...
    if key_path.exists():
        ...
        if not pem.startswith("-----BEGIN") or "PRIVATE KEY-----" not in pem:
            raise TypeError(
                f"JAAS key file at {key_path} does not contain a valid PEM private key"
            )
        return pem
...
def generate_jaas_token(room_name, user_id="dev_tester1", user_name="Developer"):
    ...
    if not app_id or not key_id:
        raise RuntimeError(
            "JAAS_APP_ID or JAAS_API_KEY_ID environment variables are missing"
        )
